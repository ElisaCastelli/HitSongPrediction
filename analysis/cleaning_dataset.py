...
def create_clean_dataframe():
    speech = len(song_df[song_df['result'] == 'Speech']) + len(song_df2[song_df2['result'] == 'Speech']) + len(
        song_df3[song_df3['result'] == 'Speech'])
    print(f"{speech} file to remove")
    print(f"{len(song_df) + len(song_df2) + len(song_df3)} total files")
    song_final = pd.concat([song_df[song_df['result'] != 'Speech'], song_df2[song_df2['result'] != 'Speech'],
                            song_df3[song_df3['result'] != 'Speech']], ignore_index=False, sort=False)
...
def remove_speech_files():
    file_to_remove = pd.concat([song_df[song_df['result'] == 'Speech'], song_df2[song_df2['result'] == 'Speech']],
