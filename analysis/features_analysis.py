...
class AudioFeatures:
    ...
    def compute_mel_spectrogram(self):
        ...
        try:
            ...
            fig, ax = plt.subplots()
            M_db = li.power_to_db(mel, ref=np.max)
            img = li.display.specshow(M_db, y_axis='mel', x_axis='time', ax=ax)
...
af = AudioFeatures(audio_dir="Audio", audio_filename="0h7Ld5CvgzaUN1zA3tdyPq.mp3")
af.compute_mel_spectrogram()
